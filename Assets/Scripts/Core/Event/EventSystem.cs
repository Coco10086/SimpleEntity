...
//		private readonly UnOrderMultiMap<Type, ILoadSystem> loadSystems = new UnOrderMultiMap<Type, ILoadSystem>();
		...
		public void Add(Assembly assembly)
		{
			...
			foreach (Type type in assemblies.GetTypes())
			{
				...
				//UnityEngine.Debug.Log(" Obo type1 "+objects[0].ToString());
				BaseAttribute baseAttribute = (BaseAttribute) objects[0];
					
				//UnityEngine.Debug.Log(" type1 "+baseAttribute.AttributeType.ToString());
					
				//Debug.Log(" Assembly sub type "+type);
...
//						break;
			...
			if (this.types.ContainsKey(typeof (EventAttribute)))
			{
				...
				object[] attrs = type.GetCustomAttributes(typeof(EventAttribute), false);
					...
					foreach (object attr in attrs)
					{
						...
						//Debug.Log(" ==event Name "+aEventAttribute.ToString());
						object obj = Activator.CreateInstance(type);
						IEvent iEvent = obj as IEvent;
						if (iEvent == null)
						{
							Log.LogError($"{obj.GetType().Name} 没有继承IEvent");
						}
		...
		public List<Type> GetTypes()
		{
			...
			allTypes.AddRange(assemblies.GetTypes());
			return allTypes;
		}
